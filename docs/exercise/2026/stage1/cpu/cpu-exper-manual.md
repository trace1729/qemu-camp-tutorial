...
专业阶段 CPU 方向的实验围绕 Xg233ai 自定义指令扩展展开，你需要按照 [G233 CPU 指令扩展手册][5] 给出的指令规格，在 QEMU TCG 前端完成指令翻译与实现（Decodetree 译码 → Helper 实现 → 测试验证）。
...
```bash
git remote add upstream git@github.com:gevico/gevico-classroom-qemu-camp-2026-cpu-qemu-camp.git
```
...
!!! note
    ...
    你需要熟读 [G233 CPU 指令扩展手册][5] 和测题的源码，来理解每个实验的测试意图，这会极大地方便你调试，提高开发效率。
...
所有指令的编码格式、操作数约定和伪代码，详见 [G233 CPU 指令扩展手册][5]。
...
| 测试用例 | 测试内容 |
| --- | --- |
| `test_sort` | 32 元素数组 `{3, 7, 23, 9, 81, 33, ...}`，排序前 16 个元素 |
